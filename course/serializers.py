...
from .models import Assistant_Teacher, High_Teacher

# Optional: AssistantTeacher uchun nested serializer
class AssistantTeacherSerializer(serializers.ModelSerializer):
    class Meta:
        model = Assistant_Teacher
        fields = ['id', 'full_name', 'email']  # kerakli maydonlarni qo‘shing
        ref_name = 'AssistantTeacherNested'

# Optional: HighTeacher uchun nested serializer
class HighTeacherSerializer(serializers.ModelSerializer):
    class Meta:
        model = High_Teacher
        fields = ['id', 'full_name', 'email']
        ref_name = 'HighTeacherNested'


# NotionURL serializer
class NotionURLSerializer(serializers.ModelSerializer):
    main_teacher = serializers.StringRelatedField()  # ustoz nomi va ma’lumotlarini ko‘rsatish
    assistant_teacher = serializers.StringRelatedField(many=True)  # yordamchi ustozlar
...
class TeacherCommentSerializer(serializers.ModelSerializer):
    ...
    task_title = serializers.CharField(
        source='task.video_lesson.title',
        read_only=True
    )
