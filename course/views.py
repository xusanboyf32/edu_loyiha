...
from .serializers import (
    ...
    GroupSerializer, VideoLessonSerializer, TaskSerializer,
    NotionURLSerializer, TeacherCommentSerializer, KnescopeVideoUrlSerializer
)
...
class HighTeacherViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [IsAuthenticated, High_Teacher]
...
class AssistantTeacherViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [IsAuthenticated, Assistant_Teacher]
    filter_backends = [filters.SearchFilter]
    search_fields = ['full_name', 'email']
...
class VideoLessonViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # -----xxx---------
        elif hasattr(user, 'role') and user.role in ['staff', 'head_teacher', 'assistant_teacher']:
            return queryset
...
class TaskViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=['get'], permission_classes=[IsAuthenticated, Assistant_Teacher])
...
# NOTIN UCHUN VIEWSET HIGH HAMDA ASSISTANT TEACHERLAR NOTION URL QOSHA OLADI
...
class NotionURLViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        # Assistant Teacher bo'lsa
        elif hasattr(user, 'assistant_teacher_profile'):
            notion = serializer.save()
            notion.assistant_teacher.add(user.assistant_teacher_profile)
        else:
            raise PermissionError("Sizga Notion qo'shish huquqi berilmagan")
...
class TeacherCommentViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        if hasattr(self.request.user, 'assistant_teacher_profile'):
            serializer.save(assistant_teacher=self.request.user.assistant_teacher_profile)
        else:
            # Admin uchun, assistant_teacher bo'sh qolishi mumkin
            serializer.save()

    def get_queryset(self):
        user = self.request.user
        if hasattr(user, 'student_profile'):
            # Talaba faqat o'z vazifasiga yozilgan commentlarni ko'radi
            return Teachercomment.objects.filter(task__student=user.student_profile)
        elif hasattr(user, 'assistant_teacher_profile') or user.is_superuser:
